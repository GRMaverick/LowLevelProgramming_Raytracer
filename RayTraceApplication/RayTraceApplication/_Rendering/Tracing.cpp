...
Vector3 Tracing::Trace(const Vector3 &rayorig, const Vector3 &raydir, Sphere* spheres[], int sphereCount, const int &depth)
{
	float tnear = INFINITY;
	const Sphere* sphere = NULL;
	...
	if ((sphere->Transparency > 0 || sphere->Reflection > 0) && depth < MAX_RAY_DEPTH)
	{
		...
		Vector3 refldir = raydir - nhit * 2 * raydir.Dot(nhit);
		refldir.Normalise();
		Vector3 reflection = Trace(phit + nhit * bias, refldir, spheres, sphereCount, depth + 1);
		Vector3 refraction = 0;
		...
		if (sphere->Transparency)
		{
			...
			Vector3 refrdir = raydir * eta + nhit * (eta *  cosi - sqrt(k));
			refrdir.Normalise();
			refraction = Trace(phit - nhit * bias, refrdir, spheres, sphereCount, depth + 1);
		}
